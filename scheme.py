...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        def one_arug_eval(expr):
            return scheme_eval(expr,env)
        procedure = scheme_eval(first, env)
        args = rest.map(one_arug_eval)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        childFrame.bindings = dict(self.bindings)

        curFormals = formals
        curVals = vals
        while curFormals != nil:
            childFrame.define(curFormals.first, curVals.first)
            curFormals = curFormals.rest
            curVals = curVals.rest
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        func = target.first
        newexp = Pair(target.rest,expressions.rest)
        env.define(func, do_lambda_form(newexp,env))
        return func
        # END PROBLEM 9
    else:
